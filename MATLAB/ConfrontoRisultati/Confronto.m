...
[f16_v, msg16_v] = fopen('file16_post_implementation.txt','r');
...
[f8_v, msg8_v] = fopen('file8_postimplementation.txt','r');
...
Res16=imboxfilt(uint32(coe16),3,'NormalizationFactor',1,'Padding',0);   % img 16 bit (64x64) -- MATLAB
...
figure(2);
subplot(1,2,1);
imshow(uint16(img16/9));
title('Immagine 16bit elaborata in VIVADO');
subplot(1,2,2);
imshow(uint16(Res16/9));
...
subplot(2,3,2);
imshow(uint8(Res8M/9));
title('Res8M');
subplot(2,3,3);
imshow(uint8(Res8L/9));
title('Res8L');
subplot(2,3,4);
imshow(uint8(Res8/9));
title('Immagine 8bit elaborata in MATLAB');
subplot(2,3,5);
imshow(uint8(img8M/9));
title('img8M');
subplot(2,3,6);
imshow(uint8(img8L/9));
title('img8L');
